...
class GestoreOrdini:
    ...
    def __init__(self):
        ...
        self._ordini_per_categoria = defaultdict(list) #se non ho quella categoria mi da lista vuota
        self._dao = DAO()
        self._allP = []
        self._allC = []
        self._fill_data()
    ...
    def _fill_data(self):
        """Leggo prodotti e clienti dal db, e poi cerco degli ordini randomici per tetstare la mia app"""
    ...
    def _update_DB(self, prod, cliente): #Aggiunge Cliente e Prodotto al DB
    ...
    def processa_prossimo_ordine(self):
        ...
        print(f"Sto processano l'ordine di {ordine.cliente}")
    ...
    def processa_tutti_ordini(self):
        ...
        while self._ordini_da_processare:
            _, ordine = self.processa_prossimo_ordine() # _ variabile che non useremo
    ...
    def get_distribuzione_categorie(self):
        ...
        valori = []
        for categoria in self._ordini_per_categoria.keys():
            ordini = self._ordini_per_categoria[categoria]
            totale_fatturato = sum([o.totale_lordo(0.22) for o in ordini])
            valori.append((categoria, totale_fatturato))
        return valori

    def get_riepilogo(self):
        """restituisce stringa con le info di massima"""
        sommario = ""
        sommario += ("\n" + "=" * 60)
...
def test_modulo():
    ...
    ordini = [
        ...
        Ordine([
                ...
                RigaOrdine(ProdottoRecord("Mouse", 10.0), 2)],
                ClienteRecord("Giuseppe Averta", "giuseppe@gmail.com", "Silver")),
        Ordine([
                RigaOrdine(ProdottoRecord("Tablet", 900.0), 1),
                RigaOrdine(ProdottoRecord("Cuffie", 250.0), 3)],
                ClienteRecord("Carlo Masone", "carlo@gmail.com", "Gold")),
        Ordine([
                RigaOrdine(ProdottoRecord("Tablet", 500.0), 1),
                RigaOrdine(ProdottoRecord("Cuffie", 250.0), 3)],
                ClienteRecord("Francesca Pistilli", "fra@gmail.com", "Bronze"))
    ]
